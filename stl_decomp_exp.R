...
write.csv(rev(myTS), "./data/simulated_exchange_rate/ts_1.csv", row.names = FALSE)
myTS <- read.csv("./data/simulated_exchange_rate/ts_1.csv")
...
for (horizon in 1:no_horizons){
  ...
  for (imf in 1:emd_output$nimf){
    ...
    final_window <- final_windows[[imf]]
    # }else{
    #   final_window <- emd_output$imf[(nrow(emd_output$imf) - input_size + 1):nrow(emd_output$imf), imf]
    #   final_window <- t(as.matrix(final_window))
    # }
...
write.csv(naive_mae, "./results/STL_Decomp/errors/naive_mae.csv", row.names = FALSE)
write.csv(leakage_mae, "./results/STL_Decomp/errors/leakage_mae.csv", row.names = FALSE)
write.csv(no_leakage_mae, "./results/STL_Decomp/errors/no_leakage_mae.csv", row.names = FALSE)

write.csv(naive_rmse, "./results/STL_Decomp/errors/naive_rmse.csv", row.names = FALSE)
write.csv(leakage_rmse, "./results/STL_Decomp/errors/leakage_rmse.csv", row.names = FALSE)
write.csv(no_leakage_rmse, "./results/STL_Decomp/errors/no_leakage_rmse.csv", row.names = FALSE)